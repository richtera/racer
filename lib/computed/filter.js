...
var fieldPredicates = {
    equals: function (fieldName, val, doc) {
      if (fieldName === '$solr' || fieldName === 'solr_query') {
        var value = doc.$solr || doc.solr_query;
        if (!value && (val === '*:*')) {
          return true; // For all query we need to make sure new docs match.
        }
        return deepEqual(doc.$solr || doc.solr_query, val);
      }
