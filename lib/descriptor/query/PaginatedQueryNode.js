...
PaginatedQueryNode.prototype.shouldPublish = function (newDoc, oldDoc, txn, store, cb, searched) {
  ...
  if (oldDocPasses && !searched) {
    ...
    if (!found) {
      ...
      return this.results(store._db, function (err, results) {
        if (err) return cb(err);
        return self.shouldPublish(newDoc, null, txn, store, function (err, messages) {
          cb(err, messages);
        }, true);
      });
    }
